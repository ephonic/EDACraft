...
from rtlgen.verify.module_adapter import normalize_executable_module, require_single_clock_module


REFERENCE_RUNTIME_ARTIFACT = "rtlgen_ref_runtime.py"
...
def write_uvm_runtime_bundle(
    ...
    *,
    ...
    """Materialize one runnable UVM bundle and optionally vendor rtlgen."""
...
def load_generated_reference_model(
    ...
    *,
    ...
    When the generated reference model and its ``rtlgen_ref_runtime.py``
    ...
    runtime location. This sets the ``RTLGEN_REF_RUNTIME_PATH`` environment
    variable for the duration of the import so the generated
    ``_load_runtime_module()`` honors it.
    """

    path = Path(ref_model_path)
    spec = importlib.util.spec_from_file_location(f"{path.stem}_rtlgen_generated_ref", path)
    if spec is None or spec.loader is None:
        raise ImportError(f"unable to load generated reference model from {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    saved_env = os.environ.get("RTLGEN_REF_RUNTIME_PATH")
    ...
    cached_runtime = sys.modules.pop("rtlgen_ref_runtime_rtlgen_runtime", None)
    if runtime_path is not None:
        os.environ["RTLGEN_REF_RUNTIME_PATH"] = str(runtime_path)
    try:
        spec.loader.exec_module(module)
        ...
        sys.modules.pop("rtlgen_ref_runtime_rtlgen_runtime", None)
        raise
    finally:
        if runtime_path is not None:
            if saved_env is None:
                os.environ.pop("RTLGEN_REF_RUNTIME_PATH", None)
            else:
                os.environ["RTLGEN_REF_RUNTIME_PATH"] = saved_env
...
def probe_iverilog_uvm_collateral(
    ...
    *,
    ...
    if output_dir is None:
        temp_dir_obj = tempfile.TemporaryDirectory(prefix="rtlgen_iverilog_probe_")
...
def _emit_interface_sv(
    ...
    if len(clock_names) > 1:
        ...
        body = "\n".join(port_lines + meta_lines + ["  event rtlgen_step_done;"])
        return (
...
def _emit_scoreboard_sv(
    ...
    return (
        ...
        f"    rtlgen_predict({_sv_string_literal(reference_model_path)}, {predict_args});\n"
...
def _emit_scoreboard_dpi_import_sv(
    ...
    return (
        "  import \"DPI-C\" context function void rtlgen_predict(\n"
...
def _emit_dsl_dut_sv(module: Any) -> str:
    ...
    if emitter_cls is None or profile_cls is None:
        from rtlgen.dsl import EmitProfile as RtlgenXEmitProfile, VerilogEmitter as RtlgenXVerilogEmitter
...
def _emit_dpi_bridge_python(
    ...
    return (
        ...
        "    module_name = resolved.stem + \"_rtlgen_ref\"\n"
...
def _emit_dpi_bridge_c(
    ...
    return (
        ...
        "#include <stdlib.h>\n"
        "#include <stdint.h>\n\n"
        f'#define RTLGEN_DPI_PY_MODULE "{stem}_dpi_bridge"\n'
        '#ifndef RTLGEN_DPI_MODULE_DIR\n'
        '#define RTLGEN_DPI_MODULE_DIR "."\n'
        "#endif\n\n"
        "static PyObject* rtlgen_bridge_module = NULL;\n"
        "static PyObject* rtlgen_predict_fn = NULL;\n\n"
        "static PyObject* rtlgen_pyint_from_svbitvec(const svBitVecVal* value, unsigned words) {\n"
        ...
        "  return py_value;\n"
        "}\n\n"
        "static void rtlgen_zero_svbitvec(svBitVecVal* value, unsigned words) {\n"
        "  unsigned idx;\n"
        "  for (idx = 0; idx < words; ++idx) {\n"
        "    value[idx] = 0u;\n"
        "  }\n"
        "}\n\n"
        "static int rtlgen_svbitvec_from_pyint(svBitVecVal* value, unsigned words, PyObject* py_value) {\n"
        "  unsigned idx;\n"
        "  for (idx = 0; idx < words; ++idx) {\n"
        ...
        "  return 1;\n"
        "}\n\n"
        "static int rtlgen_init_bridge(void) {\n"
        "  PyObject* module_dir;\n"
        "  PyObject* sys_path;\n"
        ...
        "    Py_Initialize();\n"
        "  }\n"
        "  if (rtlgen_predict_fn != NULL) {\n"
        "    return 1;\n"
        "  }\n"
        "  sys_path = PySys_GetObject(\"path\");\n"
        "  module_dir = PyUnicode_FromString(RTLGEN_DPI_MODULE_DIR);\n"
        "  if (module_dir == NULL) {\n"
        "    PyErr_Print();\n"
        "    return 0;\n"
        "  }\n"
        ...
        "  module_name = PyUnicode_FromString(RTLGEN_DPI_PY_MODULE);\n"
        "  if (module_name == NULL) {\n"
        "    PyErr_Print();\n"
        "    return 0;\n"
        "  }\n"
        "  rtlgen_bridge_module = PyImport_Import(module_name);\n"
        "  Py_DECREF(module_name);\n"
        "  if (rtlgen_bridge_module == NULL) {\n"
        "    PyErr_Print();\n"
        "    return 0;\n"
        "  }\n"
        "  rtlgen_predict_fn = PyObject_GetAttrString(rtlgen_bridge_module, \"predict_flat\");\n"
        "  if (rtlgen_predict_fn == NULL || !PyCallable_Check(rtlgen_predict_fn)) {\n"
        "    PyErr_Print();\n"
        "    return 0;\n"
        "  }\n"
        "  return 1;\n"
        "}\n\n"
        f"void rtlgen_predict(\n{func_args}\n) {{\n"
        "  PyObject* args;\n"
        "  PyObject* result;\n"
        "  if (!rtlgen_init_bridge()) {\n"
        f"{_c_zero_outputs(outputs, '    ')}"
        "    return;\n"
        "  }\n"
        f"{tuple_build}\n"
        "  result = PyObject_CallObject(rtlgen_predict_fn, args);\n"
        "  Py_DECREF(args);\n"
        f"{result_parse}\n"
        "  Py_DECREF(result);\n"
        "}\n"
...
def _active_domain_field_name(domain_name: str) -> str:
    return f"rtlgen_active_{domain_name}"
...
def _c_input_to_pyobject(port: VerificationPort) -> str:
    ...
    return f"rtlgen_pyint_from_svbitvec({port.name}, {_sv_word_count(port.width)})"
...
def _c_output_from_result(port: VerificationPort, idx: int) -> str:
    ...
    return (
        f"{{ PyObject* item_{idx} = PySequence_GetItem(result, {idx}); "
        f"if (item_{idx} == NULL) {{ PyErr_Print(); {_c_zero_outputs_inline((port,))} return; }} "
        f"if (!rtlgen_svbitvec_from_pyint(predicted_{port.name}, {_sv_word_count(port.width)}, item_{idx})) "
...
def _c_zero_outputs(outputs: Sequence[VerificationPort], indent: str) -> str:
    return "".join(
        (
            ...
            if port.width == 1
            else f"{indent}rtlgen_zero_svbitvec(predicted_{port.name}, {_sv_word_count(port.width)});\n"
        )
        for port in outputs
    )
...
def _c_zero_outputs_inline(outputs: Sequence[VerificationPort]) -> str:
    return "".join(
        (
            ...
            if port.width == 1
            else f"rtlgen_zero_svbitvec(predicted_{port.name}, {_sv_word_count(port.width)}); "
        )
        for port in outputs
    )

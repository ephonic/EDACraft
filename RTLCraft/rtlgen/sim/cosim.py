...
def _use_dsl_module(module) -> bool:
    if getattr(module, "_inputs", None):
        ...
        return type(first_input).__module__.startswith("rtlgen.")
    if getattr(module, "_outputs", None):
        first_output = next(iter(module._outputs.values()))
        return type(first_output).__module__.startswith("rtlgen.")
    return False
...
def _resolve_rtl_backend(requested: str) -> str:
    ...
    if normalized == "auto":
        if _find_local_verilator() is not None:
            return "verilator"
        if _find_local_vcs() is not None:
            return "vcs"
        return "iverilog"
    return normalized
...
def _prepare_external_sim_root(
    ...
    tempdir = tempfile.TemporaryDirectory(prefix=f"rtlgen_{backend}_cosim_")
...
def _collect_external_artifact_bundle(module, dut_src: str) -> _ExternalArtifactBundle:
    ...
    from rtlgen.dsl import collect_external_verilog_artifacts
...
def _compile_and_run_with_iverilog(
    *,
    ...
    with TemporaryDirectory(prefix="rtlgen_cosim_iverilog_") as tmpdir:
        root = Path(tmpdir)
        source_files = _artifact_source_file_map(tb_sv, dut_src, artifacts)
        for filename, contents in source_files.items():
            dest = root / filename

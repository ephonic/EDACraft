...
from rtlgen.dsl.core import Array, Context, Input, Module, Output, Reg, Signal, Wire
from rtlgen.dsl.logic import Else, If, Mux
...
class ShiftReg(Module):
    """Parameterised shift register with asynchronous active-low reset.

    Generates a compact unpacked-array declaration instead of N individual
    ``reg`` lines:
    ...
    def __init__(self, width: int, depth: int, name: str = "ShiftReg"):
        ...
        # Use Array to generate a single unpacked-array declaration
        self.regs = Array(width, depth, "regs", vtype=Reg)

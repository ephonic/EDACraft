...
from rtlgen.dsl.core import Array, EnumType, Input, Memory, Module, Output, Parameter, Reg, Signal, Wire
from rtlgen.dsl.logic import Cat, Const, Else, Elif, If, Mux, Switch
...
# ---------------------------------------------------------------------
# Ready/valid channel helpers
# ---------------------------------------------------------------------

class SkidBuffer(Module):
    """Single-entry ready/valid skid buffer with bypass.

    This is a small protocol-aware component for decoupling one cycle of
    backpressure while keeping zero additional latency in the empty/bypass case.
    """

    def __init__(self, width: int = 32, name: str = "SkidBuffer"):
        super().__init__(name)
        self.width = width
        ...
        self.in_data = Input(width, "in_data")
        self.in_valid = Input(1, "in_valid")
        self.in_ready = Output(1, "in_ready")

        self.out_data = Output(width, "out_data")
        self.out_valid = Output(1, "out_valid")
        self.out_ready = Input(1, "out_ready")

        self._buf_valid = Reg(1, "buf_valid")
        self._buf_data = Reg(width, "buf_data")

        with self.comb:
            self.in_ready <<= (self._buf_valid == 0) | self.out_ready
            self.out_valid <<= self._buf_valid | self.in_valid
            with If(self._buf_valid == 1):
                self.out_data <<= self._buf_data
            with Else():
        ...
        with self.seq(self.clk, self.rst):
            ...
            with Else():
                with If(self._buf_valid == 1):
                    with If(self.out_ready == 1):
                        with If(self.in_valid == 1):
                            self._buf_valid <<= 1
                            self._buf_data <<= self.in_data
                        with Else():
                            self._buf_valid <<= 0
                with Else():
                    with If((self.in_valid == 1) & (self.out_ready == 0)):
                        self._buf_valid <<= 1
                        self._buf_data <<= self.in_data


class ReadyValidRegister(Module):
    """Single-stage ready/valid register slice.

    This is a one-entry protocol-aware pipeline stage with backpressure.
    Unlike ``SkidBuffer``, it always inserts one registered stage. Unlike
    ...
    def __init__(
        ...
        *,
        ...
        self._data_reg = Reg(width, "data_reg")
        self._valid_reg = Reg(1, "valid_reg")

        with self.comb:
            self.in_ready <<= (self._valid_reg == 0) | self.out_ready
            self.out_valid <<= self._valid_reg
            self.out_data <<= self._data_reg
        ...
        with self.seq(self.clk, self.rst):
            with If(self.rst == 1):
                ...
                self._valid_reg <<= 0
            with Else():
                with If(self.in_ready == 1):
                    self._valid_reg <<= self.in_valid
                    with If(self.in_valid == 1):
...
class ReadyValidFIFO(Module):
    """Single-clock ready/valid FIFO with occupancy tracking.

    This is a protocol-aware queue primitive for buffering multiple beats behind
    a ready/valid interface. It supports concurrent dequeue/enqueue so a full
    queue can still accept a new beat when the output side is consuming one in
    the same cycle.
    """

    def __init__(self, width: int = 32, depth: int = 4, name: str = "ReadyValidFIFO"):
        if int(depth) < 1:
            raise ValueError("ReadyValidFIFO depth must be >= 1")
        super().__init__(name)
        self.width = width
        self.depth = depth
        ...
        self._wr_ptr = Reg(addr_w, "wr_ptr")
        self._rd_ptr = Reg(addr_w, "rd_ptr")
        self._count = Reg(level_w, "count")
        self._push_fire = Wire(1, "push_fire")
        self._pop_fire = Wire(1, "pop_fire")

        with self.comb:
            self.out_valid <<= self._count != 0
            with If(self._count != 0):
                self.out_data <<= self._storage[self._rd_ptr]
            with Else():
                self.out_data <<= 0
            self.in_ready <<= (self._count < depth) | ((self._count != 0) & self.out_ready)
            self.level <<= self._count
            self._pop_fire <<= (self._count != 0) & self.out_ready
            self._push_fire <<= self.in_valid & self.in_ready
        ...
        with self.seq(self.clk, self.rst):
            ...
            with Else():
                with If(self._push_fire == 1):
                    self._storage[self._wr_ptr] <<= self.in_data
                    with If(self._wr_ptr == (depth - 1)):
                        self._wr_ptr <<= 0
                    with Else():
                        self._wr_ptr <<= self._wr_ptr + 1

                with If(self._pop_fire == 1):
                    with If(self._rd_ptr == (depth - 1)):
                        self._rd_ptr <<= 0
                    with Else():
                        self._rd_ptr <<= self._rd_ptr + 1

                with If((self._push_fire == 1) & (self._pop_fire == 0)):
                    self._count <<= self._count + 1
                with Else():
                    with If((self._push_fire == 0) & (self._pop_fire == 1)):
                        self._count <<= self._count - 1
...
class ReqRspQueue(Module):
    ...
    The request path is buffered as a FIFO so an upstream requester can run
    ahead of a slower downstream responder. The response path currently remains
    combinationally passthrough, which keeps the component lightweight while
    still making request-side decoupling explicit.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if int(depth) < 1:
            raise ValueError("ReqRspQueue depth must be >= 1")
        super().__init__(name)
        self.req_width = int(req_width)
        self.rsp_width = int(rsp_width)
        self.depth = int(depth)
        ...
        if self.addr_width > 0:
            self.up_addr = Input(self.addr_width, "up_addr")
            self.down_addr = Output(self.addr_width, "down_addr")
        self.up_req = Input(self.req_width, "up_req")
        if self.write_enable:
            self.up_write = Input(1, "up_write")
            self.down_write = Output(1, "down_write")
        if self.strobe_width > 0:
            self.up_strb = Input(self.strobe_width, "up_strb")
            self.down_strb = Output(self.strobe_width, "down_strb")
        self.up_req_valid = Input(1, "up_req_valid")
        self.up_req_ready = Output(1, "up_req_ready")
        self.up_rsp = Output(self.rsp_width, "up_rsp")
        self.up_rsp_valid = Output(1, "up_rsp_valid")
        self.up_rsp_ready = Input(1, "up_rsp_ready")

        self.down_req = Output(self.req_width, "down_req")
        self.down_req_valid = Output(1, "down_req_valid")
        self.down_req_ready = Input(1, "down_req_ready")
        self.down_rsp = Input(self.rsp_width, "down_rsp")
        self.down_rsp_valid = Input(1, "down_rsp_valid")
        self.down_rsp_ready = Output(1, "down_rsp_ready")
        ...
        with self.comb:
            ...
            self.up_req_ready <<= (self._count < depth) | ((self._count != 0) & self.down_req_ready)
            self.level <<= self._count
            self._pop_fire <<= (self._count != 0) & self.down_req_ready
            self._push_fire <<= self.up_req_valid & self.up_req_ready

            # Response path is currently a straight passthrough.
            self.up_rsp <<= self.down_rsp
            self.up_rsp_valid <<= self.down_rsp_valid
            self.down_rsp_ready <<= self.up_rsp_ready
...
class APBRegisterBank(Module):
    """Minimal APB register bank backed by byte-enable memory storage."""
    ...
    def __init__(
        ...
        *,
        ...
        name: str = "APBRegisterBank",
    ):
        super().__init__(name)
        if data_width % 8 != 0:
            raise ValueError("APBRegisterBank requires data_width divisible by 8")
        if depth < 1:
            raise ValueError("APBRegisterBank requires depth >= 1")
        ...
        self.presetn = Input(1, "presetn")
        self.psel = Input(1, "psel")
        self.penable = Input(1, "penable")
        self.pwrite = Input(1, "pwrite")
        self.paddr = Input(self.addr_width, "paddr")
        self.pwdata = Input(self.data_width, "pwdata")
        self.prdata = Output(self.data_width, "prdata")
        self.pready = Output(1, "pready")
        self.pslverr = Output(1, "pslverr")
        self.pprot = Input(3, "pprot")
        self.pstrb = Input(self.byte_lane_width, "pstrb")

        self._rdata = Reg(self.data_width, "rdata_reg")
        ...
        word_addr = (self.paddr >> self.word_shift)[index_width - 1 : 0]
        access_fire = self.psel & self.penable

        @self.comb
        def _comb():
            self.prdata <<= self._rdata
            self.pready <<= access_fire
            self.pslverr <<= 0

        @self.seq(self.pclk, self.presetn, reset_async=True, reset_active_low=True)
        def _seq():
            with If(self.presetn == 0):
                self._rdata <<= 0
            with Else():
                with If(access_fire == 1):
                    with If(self.pwrite == 1):
                        self.regmem.write(word_addr, self.pwdata, byte_enable=self.pstrb)
                    with Else():
                        self._rdata <<= self.regmem[word_addr]
...
class AXI4LiteRegisterBank(Module):
    """Minimal AXI4-Lite register bank backed by byte-enable memory storage."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(name)
        if data_width % 8 != 0:
            raise ValueError("AXI4LiteRegisterBank requires data_width divisible by 8")
        if depth < 1:
            raise ValueError("AXI4LiteRegisterBank requires depth >= 1")
        ...
        self.clk = Input(1, "clk")
        self.rst = Input(1, "rst")
        self.awaddr = Input(self.addr_width, "awaddr")
        self.awvalid = Input(1, "awvalid")
        self.awready = Output(1, "awready")
        self.awprot = Input(3, "awprot")
        self.wdata = Input(self.data_width, "wdata")
        self.wstrb = Input(self.byte_lane_width, "wstrb")
        self.wvalid = Input(1, "wvalid")
        self.wready = Output(1, "wready")
        self.bresp = Output(2, "bresp")
        self.bvalid = Output(1, "bvalid")
        self.bready = Input(1, "bready")
        self.araddr = Input(self.addr_width, "araddr")
        self.arvalid = Input(1, "arvalid")
        self.arready = Output(1, "arready")
        self.arprot = Input(3, "arprot")
        self.rdata = Output(self.data_width, "rdata")
        self.rresp = Output(2, "rresp")
        self.rvalid = Output(1, "rvalid")
        self.rready = Input(1, "rready")

        self._aw_seen = Reg(1, "aw_seen")
        self._aw_addr = Reg(self.addr_width, "aw_addr_latched")
        self._w_seen = Reg(1, "w_seen")
        self._w_data = Reg(self.data_width, "w_data_latched")
        self._w_strb = Reg(self.byte_lane_width, "w_strb_latched")
        self._ar_addr = Reg(self.addr_width, "ar_addr_latched")
        self._bvalid = Reg(1, "bvalid_state")
        self._rvalid = Reg(1, "rvalid_state")
        self._rdata = Reg(self.data_width, "rdata_state")
        self._aw_capture = Wire(1, "aw_capture")
        self._w_capture = Wire(1, "w_capture")
        self._write_commit = Wire(1, "write_commit")
        ...
        merged_awaddr = Mux(self._aw_seen, self._aw_addr, self.awaddr)
        merged_wdata = Mux(self._w_seen, self._w_data, self.wdata)
        merged_wstrb = Mux(self._w_seen, self._w_strb, self.wstrb)
        write_addr = (merged_awaddr >> self.word_shift)[index_width - 1 : 0]
        read_addr = (self.araddr >> self.word_shift)[index_width - 1 : 0]

        @self.comb
        def _comb():
            self.awready <<= (~self._aw_seen) & (~self._bvalid)
            self.wready <<= (~self._w_seen) & (~self._bvalid)
            self.arready <<= ~self._rvalid
            self.bresp <<= 0
            self.rresp <<= 0
            self.bvalid <<= self._bvalid
            self.rvalid <<= self._rvalid
            self.rdata <<= self._rdata
            self._aw_capture <<= self.awvalid & self.awready
            self._w_capture <<= self.wvalid & self.wready
            self._write_commit <<= (
                (self._aw_seen | self._aw_capture)
                & (self._w_seen | self._w_capture)
                & (~self._bvalid)
            )
...
class WishboneRegisterBank(Module):
    """Clocked Wishbone register bank with byte-enable storage updates."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(name)
        if data_width % 8 != 0:
            raise ValueError("WishboneRegisterBank requires data_width divisible by 8")
        if depth < 1:
            raise ValueError("WishboneRegisterBank requires depth >= 1")
        ...
        self.adr_i = Input(self.addr_width, "adr_i")
        self.dat_i = Input(self.data_width, "dat_i")
        self.we_i = Input(1, "we_i")
        self.sel_i = Input(self.byte_lane_width, "sel_i")
        self.stb_i = Input(1, "stb_i")
        self.cyc_i = Input(1, "cyc_i")
        self.cti_i = Input(3, "cti_i")
        self.bte_i = Input(2, "bte_i")
        self.dat_o = Output(self.data_width, "dat_o")
        self.ack_o = Output(1, "ack_o")
        self.err_o = Output(1, "err_o")
        self.rty_o = Output(1, "rty_o")

        self._ack = Reg(1, "ack_state")
        self._read_valid = Reg(1, "read_valid_state")
        self._read_data = Reg(self.data_width, "read_data_state")
        self._addr_latched = Reg(self.addr_width, "addr_latched")
        self._wdata_latched = Reg(self.data_width, "wdata_latched")
        self._sel_latched = Reg(self.byte_lane_width, "sel_latched")
        self._read_fire_d = Reg(1, "read_fire_d")
        self._write_fire_d = Reg(1, "write_fire_d")
        self._access_fire = Wire(1, "access_fire")
        ...
        latched_word_addr = (self._addr_latched >> self.word_shift)[index_width - 1 : 0]

        @self.comb
        def _comb():
            self._access_fire <<= self.cyc_i & self.stb_i

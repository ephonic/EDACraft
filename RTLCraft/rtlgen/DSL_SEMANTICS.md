# rtlgen DSL Semantic Contract

This document describes the canonical semantic contract for `rtlgen.dsl`.
...
`rtlgen.dsl` is the only supported high-level authoring surface in
`rtlgen`.
...
The DSL is meant to feed the rest of `rtlgen` without forcing users to manage
another public IR.
...
`rtlgen` is expected to preserve enough source identity that findings can be

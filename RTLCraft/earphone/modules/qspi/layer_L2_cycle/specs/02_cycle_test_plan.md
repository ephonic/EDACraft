# EarphoneQSPI — L2 cycle — Verification Test Plan

| Document ID | QSPI-L2_CYCLE-TP-001 |
|-------------|--------------|
| Version     | 0.1 |
| Date        | 2026-06-18 |
...
### 1.1 Purpose
Verification test plan for the L2 cycle of EarphoneQSPI.
...
|-------------|-------|---------|
| QSPI-L2_CYCLE-001 | EarphoneQSPI L2 cycle specification | 0.1 |
...
| DUT Version | 0.1 |
| Hierarchy Path | earphone.modules.qspi.layer_L2_cycle |
...
Run the pytest cases listed in `layer_L2_cycle/specs/02_cycle_test_plan.md` under `earphone/modules/qspi/layer_L2_cycle/tests`, then publish PASS/FAIL evidence in `layer_L2_cycle/specs/02_cycle_test_report.md`. Test intent: Consumes approved outputs from `QSPI-L1_BEHAVIOR-001` (`layer_L1_behavior/specs/01_behavior_spec.md`), plus verification intent `QSPI-L1_BEHAVIOR-TP-001` (`layer_L1_behavior/specs/01_behavior_test_plan.md`) and latest evidence `QSPI-L1_BEHAVIOR-TR-001` (`layer_L1_behavior/specs/01_behavior_test_report.md`).
...
| TC ID | Name | Type | Priority | Objective | Status |
| --- | --- | --- | --- | --- | --- |
| QSPI-L2_CYCLE-TC-001 | test_describe | Directed | P1 | Validate describe. | Planned |
| QSPI-L2_CYCLE-TC-002 | test_xip_read_fsm_completes | Directed | P1 | Validate xip read fsm completes. | Planned |
...
|-------------|------|-----------|-------|
| local-pytest | pytest | per flow run | EarphoneQSPI L2 cycle |

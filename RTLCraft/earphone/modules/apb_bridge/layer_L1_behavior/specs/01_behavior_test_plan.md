# EarphoneAPBBridge — L1 behavior — Verification Test Plan

| Document ID | APB_BRIDGE-L1_BEHAVIOR-TP-001 |
|-------------|--------------|
| Version     | 0.1 |
| Date        | 2026-06-18 |
...
### 1.1 Purpose
Verification test plan for the L1 behavior of EarphoneAPBBridge.
...
|-------------|-------|---------|
| APB_BRIDGE-L1_BEHAVIOR-001 | EarphoneAPBBridge L1 behavior specification | 0.1 |
...
| DUT Version | 0.1 |
| Hierarchy Path | earphone.modules.apb_bridge.layer_L1_behavior |
...
Run the pytest cases listed in `layer_L1_behavior/specs/01_behavior_test_plan.md` under `earphone/modules/apb_bridge/layer_L1_behavior/tests`, then publish PASS/FAIL evidence in `layer_L1_behavior/specs/01_behavior_test_report.md`. Test intent: Consumes the module contract `APB_BRIDGE-MOD-001` and top-level SoC requirements as the seed SpecIR.
...
|-------|------|------|----------|-----------|--------|
| TC-001 | test_decode_each_slot_base | Directed | P1 | Validate decode each slot base. | Planned |
...
| TC ID | Name | Type | Priority | Objective | Status |
| --- | --- | --- | --- | --- | --- |
| APB_BRIDGE-L1_BEHAVIOR-TC-001 | test_decode_each_slot_base | Directed | P1 | Validate decode each slot base. | Planned |
| APB_BRIDGE-L1_BEHAVIOR-TC-002 | test_decode_out_of_range_fallback | Directed | P1 | Validate decode out of range fallback. | Planned |
| APB_BRIDGE-L1_BEHAVIOR-TC-003 | test_decode_slot_internal_offset | Directed | P1 | Validate decode slot internal offset. | Planned |
| APB_BRIDGE-L1_BEHAVIOR-TC-004 | test_describe | Directed | P1 | Validate describe. | Planned |

### 7.2 Detailed Test Cases

#### TC-001: test_decode_each_slot_base
| Attribute | Description |
|-----------|-------------|
| Objective | Validate decode each slot base. |
...
|-------------|------|-----------|-------|
| local-pytest | pytest | per flow run | EarphoneAPBBridge L1 behavior |

"""Test JpegDecoder with C++ compiled simulator backend."""
...
def run_sim(sim, coeffs, max_cycles=5000):
...
def main():
    ...
    py_y = np.array([(p >> 16) & 0xFF for p in py_pixels[:64]], dtype=np.uint8).reshape(8, 8)
    print(f"Python: {py_cycles} cycles, match={np.array_equal(py_y, expected)}")

    print("Running C++ compiled simulator...")
    module = JpegDecoder()
    with build_compiled_simulator_from_dsl(module, build_dir="jpeg_decoder/build/cpp_sim") as cpp_sim:
        cpp_sim.reset()
        cpp_pixels, cpp_cycles = run_sim(cpp_sim, coeffs)
        cpp_y = np.array([(p >> 16) & 0xFF for p in cpp_pixels[:64]], dtype=np.uint8).reshape(8, 8)
        print(f"C++: {cpp_cycles} cycles, match={np.array_equal(cpp_y, expected)}")
        print("C++ output:")
        print(cpp_y)

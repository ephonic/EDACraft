...
def main():
    dq = JpegDequantZigzag()
    idct = JpegIdct8x8()

    # Build a wrapper module connecting dq -> idct.
    from rtlgen_x.dsl import Module, Input, Output, Reg, Wire, Cat
    ...
    class Wrapper(Module):
        def __init__(self):
            ...
            self.instantiate(dq, "u_dq", port_map={
                ...
                "out_data": Wire(COEFF_WIDTH, "dq_out"),
                "out_valid": Wire(1, "dq_out_valid"),
                "out_ready": Wire(1, "dq_out_ready"),
            })
            self.instantiate(idct, "u_idct", port_map={
                "clk": self.clk, "rst": self.rst,
                "in_data": self._u_dq_out,
                "in_valid": self._u_dq_out_valid,
                "in_ready": self._u_dq_out_ready,
                "out_data": self.out_data, "out_valid": self.out_valid, "out_ready": self.out_ready,
            })

    # Actually instantiate fresh modules inside wrapper.
    class Wrapper2(Module):
        def __init__(self):
            super().__init__("Wrapper2")
            self.clk = Input(1, "clk")
            self.rst = Input(1, "rst")
            self.in_data = Input(12, "in_data", signed=True)
            self.in_valid = Input(1, "in_valid")
            ...
            dq_out = Wire(COEFF_WIDTH, "dq_out", signed=True)
            dq_out_valid = Wire(1, "dq_out_valid")
            dq_out_ready = Wire(1, "dq_out_ready")

            self.instantiate(u_dq, "u_dq", port_map={
                "clk": self.clk, "rst": self.rst,
                "in_data": self.in_data, "in_valid": self.in_valid, "in_ready": self.in_ready,
                "out_data": dq_out, "out_valid": dq_out_valid, "out_ready": dq_out_ready,
            })
            self.instantiate(u_idct, "u_idct", port_map={
                "clk": self.clk, "rst": self.rst,
                "in_data": dq_out, "in_valid": dq_out_valid, "in_ready": dq_out_ready,
                "out_data": self.out_data, "out_valid": self.out_valid, "out_ready": self.out_ready,
            })

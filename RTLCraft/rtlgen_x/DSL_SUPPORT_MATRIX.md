...
| Multi-clock authoring | Yes | Partial | Partial | Partial | Yes | Partial | `partial` | `clock_domain(...)` / `seq_domain(...)` exist at authoring level, including direct `seq_domain("name")` binding; closure still requires explicit domain stepping or directed active-domain flows |
...
| `Memory(..., read_ports=..., write_ports=..., read_style=..., read_latency=...)` metadata | Yes | Yes | Yes | Yes | Indirect | Partial | `partial` | Storage intent is explicit and preserved; executable lowering closes single-read/single-write async-read zero-latency memories and normalizes sync-read latency-1 memories, but emitted RTL closure is still partial |

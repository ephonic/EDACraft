...
The current resolution policy in
[rtlgen_x/sim/cosim.py](/Users/yangfan/release/EDACraft-main/RTLCraft/rtlgen_x/sim/cosim.py)
is:

1. `auto -> verilator` when a local `verilator` is available
2. otherwise `auto -> vcs` when local or remote VCS is available
...
3. `vcs` when you need stronger project-style simulator behavior or remote farm integration
...
3. `test_dsl_rtl_cosim_auto_backend_prefers_remote_vcs_when_configured`
4. `test_dsl_rtl_cosim_explicit_vcs_uses_remote_when_configured`
...
```bash
PYTHONPATH=. pytest -q rtlgen_x/tests/test_cosim.py -k 'collect_external_artifact_bundle'
PYTHONPATH=. pytest -q rtlgen_x/tests/test_cosim.py -k 'compile_and_run_with_iverilog_stages_compile_flags'
PYTHONPATH=. pytest -q rtlgen_x/tests/test_cosim.py -k 'auto_backend'
```
...
Remote VCS is intentionally part of the same closure path, not a separate
workflow.

When `RTLGEN_X_REMOTE_VCS_HOST` is configured:

1. `rtl_backend="auto"` may resolve to `vcs`
2. `rtl_backend="vcs"` can use the remote compile/run path even without a local VCS install
3. artifact staging still carries the same external sources, include trees, and init files

The remote-cache and SSH pipeline behavior is regression-backed in
[rtlgen_x/tests/test_cosim.py](/Users/yangfan/release/EDACraft-main/RTLCraft/rtlgen_x/tests/test_cosim.py).

    ...
    def __init__(self, name="decoder_8b10b"):
        ...
        def _pipeline():
            with If(self.reset_in == 1):
                ...
                self.r_valid <<= 0
                self.decoder_out <<= 0
                self.decoder_valid_out <<= 0
                self.control_out <<= 0
            with Else():
                self.r_data <<= self.decoder_in
                self.r_ctrl <<= self.control_in
                self.r_valid <<= self.decoder_valid_in
                self.decoder_valid_out <<= self.r_valid
                self.control_out <<= self.r_ctrl
                with If(self.r_ctrl == 1):
                    self.decoder_out <<= self.control_dec
                with Else():
                    self.decoder_out <<= self.data_dec

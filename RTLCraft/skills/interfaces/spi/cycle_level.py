"""
skills.interfaces.spi.cycle_level — Cycle-Level Models (register-accurate)
"""
...
def pos_edge_detector_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate PosEdgeDetector model."""
...
def neg_edge_detector_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate NegEdgeDetector model."""
    def behavior(ctx: CycleContext) -> None:
        rst_n = ctx.get_input("rst_n", 1)
        if rst_n == 0:
            for o in []:
...
_template_map = {
    "neg_edge_detector": neg_edge_detector_cycle,
    "pos_edge_detector": pos_edge_detector_cycle,
    "spi_clock_divider": spi_clock_divider_cycle,
    "spi_module": spi_module_cycle,
    "spi_top": spi_top_cycle,
}

for _name, _tmpl in _template_map.items():
    TemplateRegistry.register(_name, _tmpl)

neg_edge_detector_template = neg_edge_detector_cycle
pos_edge_detector_template = pos_edge_detector_cycle
spi_clock_divider_template = spi_clock_divider_cycle
spi_module_template = spi_module_cycle
spi_top_template = spi_top_cycle

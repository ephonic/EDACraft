"""
skills.interfaces.btle.cycle_level — Cycle-Level Models (register-accurate)
"""
...
def access_address_detect_cycle(
    len_seq: int = 32,
    **kwargs,
) -> Callable[[CycleContext], None]:
    """Cycle-accurate Access Address Detect model."""
    def behavior(ctx: CycleContext):
        rst = ctx.get_input("rst", 0)
...
def bit_upsampler_cycle(
    sample_per_symbol: int = 8,
    **kwargs,
) -> Callable[[CycleContext], None]:
    """Cycle-accurate Bit Upsampler model."""
    def behavior(ctx: CycleContext):
        rst = ctx.get_input("rst", 0)
        phy_bit = ctx.get_input("phy_bit", 0)
        bit_valid = ctx.get_input("bit_valid", 0)
...
def crc_wrapper_cycle(
    crc_width: int = 24,
    payload_len_bits: int = 8,
    **kwargs,
) -> Callable[[CycleContext], None]:
    """Cycle-accurate Crc Wrapper model."""
    def behavior(ctx: CycleContext):
        rst = ctx.get_input("rst", 0)
...
def scramble_wrapper_cycle(
    channel_width: int = 6,
    payload_len_bits: int = 8,
    **kwargs,
) -> Callable[[CycleContext], None]:
    """Cycle-accurate Scramble Wrapper model."""
    def behavior(ctx: CycleContext):
        rst = ctx.get_input("rst", 0)
        ch = ctx.get_input("channel_number", 0)
        ch_load = ctx.get_input("channel_number_load", 0)
...
def search_unique_bit_seq_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate SEARCH_UNIQUE_BIT_SEQ model."""
...
def gfsk_demodulation_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate GFSK_DEMODULATION model."""
    def behavior(ctx: CycleContext) -> None:
        rst_n = ctx.get_input("rst_n", 1)
        if rst_n == 0:
            for o in []:
...
def bit_repeat_upsample_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate BIT_REPEAT_UPSAMPLE model."""
    def behavior(ctx: CycleContext) -> None:
        rst_n = ctx.get_input("rst_n", 1)
        if rst_n == 0:
            for o in []:
...
def sdpram_one_clk_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate SDPRAM_ONE_CLK model."""
    def behavior(ctx: CycleContext) -> None:
        rst_n = ctx.get_input("rst_n", 1)
        if rst_n == 0:
            for o in []:
...
def sdpram_two_clk_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate SDPRAM_TWO_CLK model."""
    def behavior(ctx: CycleContext) -> None:
        rst_n = ctx.get_input("rst_n", 1)
        if rst_n == 0:
            for o in []:
...
def crc24_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate CRC24 model."""
    def behavior(ctx: CycleContext) -> None:
        rst_n = ctx.get_input("rst_n", 1)
        if rst_n == 0:
            for o in []:
...
def scramble_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate SCRAMBLE model."""
    def behavior(ctx: CycleContext) -> None:
        rst_n = ctx.get_input("rst_n", 1)
        if rst_n == 0:
            for o in []:
...
def gfsk_modulation_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate GFSK_MODULATION model."""
    def behavior(ctx: CycleContext) -> None:
        rst_n = ctx.get_input("rst_n", 1)
        if rst_n == 0:
            for o in []:
...
_template_map = {
    "access_address_detect": access_address_detect_cycle,
    "bit_repeat_upsample": bit_repeat_upsample_cycle,
    "bit_upsampler": bit_upsampler_cycle,
    "btle_phy": btle_phy_cycle,
    "btle_rx_core": btle_rx_core_cycle,
    "btle_tx": btle_tx_cycle,
    "crc24": crc24_cycle,
    "crc24_core": crc24_core_cycle,
    "crc_wrapper": crc_wrapper_cycle,
    "gauss_filter": gauss_filter_cycle,
    "gfsk_demod": gfsk_demod_cycle,
    "gfsk_demodulation": gfsk_demodulation_cycle,
    "gfsk_mod": gfsk_mod_cycle,
    "gfsk_modulation": gfsk_modulation_cycle,
    "scramble": scramble_cycle,
    "scramble_core": scramble_core_cycle,
    "scramble_wrapper": scramble_wrapper_cycle,
    "sdpram": sdpram_cycle,
    "sdpram_one_clk": sdpram_one_clk_cycle,
    "sdpram_two_clk": sdpram_two_clk_cycle,
    "search_unique_bit_seq": search_unique_bit_seq_cycle,
    "vco": vco_cycle,
}
...
access_address_detect_template = access_address_detect_cycle
bit_repeat_upsample_template = bit_repeat_upsample_cycle
bit_upsampler_template = bit_upsampler_cycle
btle_phy_template = btle_phy_cycle
btle_rx_core_template = btle_rx_core_cycle
btle_tx_template = btle_tx_cycle
crc24_template = crc24_cycle
crc24_core_template = crc24_core_cycle
crc_wrapper_template = crc_wrapper_cycle
gauss_filter_template = gauss_filter_cycle
gfsk_demod_template = gfsk_demod_cycle
gfsk_demodulation_template = gfsk_demodulation_cycle
gfsk_mod_template = gfsk_mod_cycle
gfsk_modulation_template = gfsk_modulation_cycle
scramble_template = scramble_cycle
scramble_core_template = scramble_core_cycle
scramble_wrapper_template = scramble_wrapper_cycle
sdpram_template = sdpram_cycle
sdpram_one_clk_template = sdpram_one_clk_cycle
sdpram_two_clk_template = sdpram_two_clk_cycle
search_unique_bit_seq_template = search_unique_bit_seq_cycle
vco_template = vco_cycle

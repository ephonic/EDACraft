"""
skills.interfaces.uart.cycle_level — Cycle-Level Models (register-accurate)
"""
...
def uart_cycle(**kwargs) -> Callable[[CycleContext], None]:
    """Cycle-accurate UART model."""
...
_template_map = {
    "uart": uart_cycle,
    "uart_rx": uart_rx_cycle,
    "uart_top": uart_top_cycle,
    "uart_tx": uart_tx_cycle,
}

for _name, _tmpl in _template_map.items():
    TemplateRegistry.register(_name, _tmpl)

uart_template = uart_cycle
uart_rx_template = uart_rx_cycle
uart_top_template = uart_top_cycle
uart_tx_template = uart_tx_cycle

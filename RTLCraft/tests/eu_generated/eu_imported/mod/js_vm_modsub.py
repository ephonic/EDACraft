...
class js_vm_modsub(Module):
    def __init__(self, name: str = "js_vm_modsub"), PART: int = 4, SEG_WIDTH: int = (((self.DATA_WIDTH + self.PART) - 1) // self.PART):
        super().__init__(name or "js_vm_modsub")
        ...
        self.s0_sum_1 = Wire(((self.SEG_WIDTH - 1) - 0 + 1), "s0_sum_1")
        ...
        self.a_ext <<= Cat(Rep(Cat(0), ((self.PART * self.SEG_WIDTH) - self.DATA_WIDTH)), self.a[(self.DATA_WIDTH - 1):0])
        self.b_ext <<= Cat(Rep(Cat(1), ((self.PART * self.SEG_WIDTH) - self.DATA_WIDTH)), (not self.b[(self.DATA_WIDTH - 1):0]))
        self.q_ext <<= Cat(Rep(Cat(0), ((self.PART * self.SEG_WIDTH) - self.DATA_WIDTH)), self.q[(self.DATA_WIDTH - 1):0])
        ...
        for part in range(0, self.PART):
            ...
            with If((part == 0)):
                # TODO: unpack assignment: Cat(self.s0_carry_segment[part], self.s0_sum_1) = ((self.a_segment[part] + self.b_segment[part]) + 1)
                # Consider using Split() or manual bit slicing
                with If(self.pipe_en0):
                    self.s0_sum_segment_r[part] <<= self.s0_sum_1
                with Else():
        ...
        for part in range(0, self.PART):
            ...
            with If((part == 0)):
                # TODO: unpack assignment: Cat(self.s1_carry_segment[part], self.s1_sum_1) = (self.s0_sum_segment_r[part] + self.q_segment[part])
                # Consider using Split() or manual bit slicing
                with If(self.pipe_en1):
                    self.s1_sum_segment_r[part] <<= Mux(self.s0_carry_r, self.s0_sum_segment_r[part], self.s1_sum_1)
                with Else():
            ...
            with Else():
                ...
                with If(self.pipe_en1):
                    self.s1_sum_segment_r[part] <<= Mux(self.s0_carry_r, self.s0_sum_segment_r[part], Mux(self.s1_carry_segment[(part - 1)], self.s1_sum_segment_1[(part - 1)], self.s1_sum_segment_0[(part - 1)]))
                with Else():
                    self.s1_sum_segment_r[part] <<= self.s1_sum_segment_r[part]

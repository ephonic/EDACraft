...
class js_zk_mul_level2(Module):
    def __init__(self, name: str = "js_zk_mul_level2"), A_WIDTH: int = 96, B_WIDTH: int = 96, WIDTH: int = Mux((self.A_WIDTH > self.B_WIDTH), self.A_WIDTH, self.B_WIDTH), WIDTH_ODD: int = (self.WIDTH % 2), WIDTH_HALF: int = ((self.WIDTH - self.WIDTH_ODD) // 2), H_WIDTH: int = (self.WIDTH_HALF + self.WIDTH_ODD), L_WIDTH: int = self.WIDTH_HALF, F_WIDTH: int = (self.H_WIDTH + 1):
        super().__init__(name or "js_zk_mul_level2")
        ...
        u0_mul = js_zk_mul_level1(None=self.L_WIDTH)
        self.instantiate(
            u0_mul,
            "u0_mul",
        ...
        u1_mul = js_zk_mul_level1(None=self.H_WIDTH)
        self.instantiate(
            u1_mul,
            "u1_mul",
        ...
        u2_mul = js_zk_mul_level1(None=self.F_WIDTH)
        self.instantiate(
            u2_mul,
            "u2_mul",

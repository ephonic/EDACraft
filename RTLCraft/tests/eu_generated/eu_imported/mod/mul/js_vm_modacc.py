...
class js_vm_modacc(Module):
    def __init__(self, name: str = "js_vm_modacc"), EXT_BITS: int = 8, PART: int = 4, SEG_WIDTH: int = ((((self.DATA_WIDTH + self.EXT_BITS) + self.PART) - 1) // self.PART):
        super().__init__(name or "js_vm_modacc")
        ...
        self.data = Input(((((self.ISA_BITS + self.REG_WIDTH) + 1) - 1) - 0 + 1), "data")
        ...
        # TODO: unpack assignment: Cat(self.isa, self.cc_val, self.a) = self.data
        # Consider using Split() or manual bit slicing
        self.i_dst_fmt <<= self.isa[82:79]
        self.add_a <<= self.a
        self.add_b <<= self.acc_reg[self.i_vm_id]
        ...
        with ForGen("i", 0, 16) as i:
            ...
            def _seq_logic():
                ...
                with Else():
                    with If(self.pipe_en0):
                        with If(((self.i_dst_fmt == 11) & (i == self.i_vm_id))):
                            self.acc_reg[i] <<= self.acc_res
                            self.acc_count[i] <<= (self.acc_count[i] + 1)
                        with Else():
                            with If(((self.i_dst_fmt == 12) & (i == self.i_vm_id))):
                                self.acc_reg[i] <<= 0
                                self.acc_count[i] <<= 0
                    with Else():
                        with If(((self.acc_update[(self.EXT_BITS - 1)] & self.acc_overflow) & (i == self.s1_vm_id[(self.EXT_BITS - 1)]))):
                            self.acc_reg[i] <<= Cat(Rep(Cat(0), ((self.PART * self.SEG_WIDTH) - self.DATA_WIDTH)), self.res_tmp[(self.DATA_WIDTH - 1):0])
                            self.acc_count[i] <<= 0
        ...
        for redc in range(0, self.EXT_BITS):
            with If((redc == 0)):
                ...
                # for-loop (non-generate) - parameter-driven
                for part in range(0, self.PART):
                    self.to_redc_seg[part] <<= self.s0_sum_seg_r[part]

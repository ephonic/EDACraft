...
class add(Module):
    def __init__(self, name: str = "add"):
        ...
        self.add_full_in = Input(((((self.ISA_BITS + (2 * self.REG_WIDTH)) + 1) - 1) - 0 + 1), "add_full_in")
        self.add_full_out = Output(((((self.ISA_BITS + self.REG_WIDTH) + 1) - 1) - 0 + 1), "add_full_out")
        self.add_vm_id_in = Input(4, "add_vm_id_in")
        self.add_vm_id_out = Output(4, "add_vm_id_out")
        ...
        self.stage2_sum = Reg(128, "stage2_sum")
        self.stage2_carry = Reg(1, "stage2_carry")
        ...
        self.sum_sign_stage2 = Reg(1, "sum_sign_stage2")
        self.valid_stage3 = Reg(1, "valid_stage3")
        self.ready_stage3 = Reg(1, "ready_stage3")

        # TODO: unpack assignment: Cat(self.int_and_isa, self.int_and_cc_value, self.int_and_data_y, self.int_and_data_x) = self.add_full_in
        ...
        def _seq_logic():
            ...
            with Else():
                with If((self.ready_stage1 & self.valid_i)):
                    ...
                    self.valid_stage1 <<= 1
                    self.add_full_stage1 <<= self.add_full_in
                    self.vm_id_stage1 <<= self.add_vm_id_in
                with Else():
        ...
        def _seq_logic():
            ...
            with Else():
                with If((self.valid_stage1 & self.ready_stage2)):
                    ...
                    Cat(self.stage2_carry, self.stage2_sum) <<= (self.stage1_operand_a + self.stage1_operand_b)
                    self.valid_stage2 <<= 1
                    self.add_full_stage2 <<= self.add_full_stage1
        ...
        def _seq_logic():
            with If((~self.rst_n)):
                self.sum <<= 0
                self.valid_o <<= 0
                self.add_full_stage3 <<= 0
                self.vm_id_stage3 <<= 0
            with Else():
                with If((self.valid_stage2 & self.ready_stage3)):
                    self.add_full_stage3 <<= self.add_full_stage2
                    self.vm_id_stage3 <<= self.vm_id_stage2
                    self.sum <<= self.stage2_sum
                    self.valid_o <<= 1
                    with If(self.signed_mode_reg_stage2):
                        with If(((self.operand_a_sign == self.operand_b_sign) & (self.operand_a_sign != self.stage2_sum[127]))):
                            self.sum[127] <<= self.operand_a_sign
                with Else():
        ...
        # TODO: unpack assignment: Cat(self.int_and_isa_delay, self.int_and_cc_value_delay, self.int_and_data_y_delay, self.int_and_data_x_delay) = self.add_full_in_delay
        # Consider using Split() or manual bit slicing
        self.flag_bits_delay <<= self.int_and_isa_delay[25:23]
        self.signed_mode_delay <<= self.int_and_isa_delay[23]
        self.res_valid_bits <<= Mux(((self.flag_bits_delay == 0) & (self.signed_mode_delay == 1)), Cat(Rep(Cat(0), 120), self.sum[7:0]), Mux(((self.flag_bits_delay == 0) & (self.signed_mode_delay == 0)), Cat(Rep(Cat(1), 120), self.sum[7:0]), Mux(((self.flag_bits_delay == 1) & (self.signed_mode_delay == 1)), Cat(Rep(Cat(0), 112), self.sum[15:0]), Mux(((self.flag_bits_delay == 1) & (self.signed_mode_delay == 0)), Cat(Rep(Cat(1), 112), self.sum[15:0]), Mux(((self.flag_bits_delay == 2) & (self.signed_mode_delay == 1)), Cat(Rep(Cat(0), 96), self.sum[31:0]), Mux(((self.flag_bits_delay == 2) & (self.signed_mode_delay == 0)), Cat(Rep(Cat(1), 96), self.sum[31:0]), Mux(((self.flag_bits_delay == 3) & (self.signed_mode_delay == 1)), Cat(Rep(Cat(0), 64), self.sum[63:0]), Mux(((self.flag_bits_delay == 3) & (self.signed_mode_delay == 0)), Cat(Rep(Cat(1), 64), self.sum[63:0]), Mux(((self.flag_bits_delay == 4) & (self.signed_mode_delay == 1)), self.sum[127:0], Mux(((self.flag_bits_delay == 4) & (self.signed_mode_delay == 0)), self.sum[127:0], 0))))))))))
        self.add_full_in_delay <<= self.add_full_stage3
        self.add_vm_id_in_delay <<= self.vm_id_stage3
        self.add_full_out <<= Cat(self.add_full_in_delay[(((self.ISA_BITS + (2 * self.REG_WIDTH)) + 1) - 1):((((self.ISA_BITS + (2 * self.REG_WIDTH)) + 1) - 1) - (self.ISA_BITS + 1))], Rep(Cat(0), (self.REG_WIDTH - 128)), self.res_valid_bits)
        self.add_vm_id_out <<= self.add_vm_id_in_delay

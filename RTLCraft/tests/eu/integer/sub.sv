module sub (
    ...
    output reg valid_o,                  // Indicates subtraction completion
    input wire ready_i,                   // Ready to accept output
    input wire [ISA_BITS+2*REG_WIDTH+1-1:0] sub_full_in,  
    output logic [ISA_BITS+REG_WIDTH+1-1:0] sub_full_out,
    input  [3:0] sub_vm_id_in,
    output [3:0] sub_vm_id_out
);
//generate oprand_a and operand_b
logic signed_mode;              // 1: Signed , 0: Unsigned 
logic [127:0] operand_a;        // 128-bit operand A
logic [127:0] operand_b;        // 128-bit operand B
logic [127:0] difference;              // 128-bit sub result
...
assign {int_and_isa,int_and_cc_value,int_and_data_y,int_and_data_x} = sub_full_in;
assign {rsv,
        dst_fmt,
        dst_type,
        dst1_reg,
...
//delay in pipeline
logic [ISA_BITS+2*REG_WIDTH+1-1:0] sub_full_stage1, sub_full_stage2, sub_full_stage3, sub_full_in_delay;
logic [3:0] vm_id_stage1, vm_id_stage2, vm_id_stage3, sub_vm_id_in_delay;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            valid_stage1 <= 1;
            sub_full_stage1 <= sub_full_in;
            vm_id_stage1 <= sub_vm_id_in;  
    ...
    // Stage 2: Perform subtraction with handshake control
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            stage2_diff <= 128'b0;
            stage2_borrow <= 0;
            ...
            // Perform subtraction
            {stage2_borrow, stage2_diff} <= stage1_operand_a - stage1_operand_b;
            valid_stage2 <= 1;
    ...
    // Stage 3: Handle signed correction and output with handshake control
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            difference <= 128'b0;
            valid_o <= 0;
            sub_full_stage3 <= 128'd0;
            vm_id_stage3 <= 0;
            diff_sign_stage2 <= 0;
        end else if (valid_stage2 && ready_stage3) begin
            // Capture results from stage 2
            sub_full_stage3 <= sub_full_stage2;
            vm_id_stage3 <= vm_id_stage2;
            difference <= stage2_diff;
            valid_o <= 1;

            // Adjust sign for signed subtraction
            if (signed_mode_reg_stage2) begin
                diff_sign_stage2 <= stage2_diff[127];
                // If overflow occurred, adjust the sign bit to match expected result
                if ((operand_a_sign != operand_b_sign) && (operand_a_sign != diff_sign_stage2)) begin
                    difference[127] <= operand_a_sign; // Correct sign bit
                end
            end
...
assign {int_and_isa_delay,int_and_cc_value_delay,int_and_data_y_delay,int_and_data_x_delay} = sub_full_in_delay;
assign flag_bits_delay = int_and_isa_delay[25:23];
assign signed_mode_delay = int_and_isa_delay[23];

assign res_valid_bits = (flag_bits_delay==3'h0 && signed_mode_delay == 1'b1) ? {{120{1'b0}},difference[7:0]} :
                        (flag_bits_delay==3'h0 && signed_mode_delay == 1'b0) ? {{120{1'b1}},difference[7:0]} :
                        (flag_bits_delay==3'h1 && signed_mode_delay == 1'b1) ? {{112{1'b0}},difference[15:0]} :
                        (flag_bits_delay==3'h1 && signed_mode_delay == 1'b0) ? {{112{1'b1}},difference[15:0]} :
                        (flag_bits_delay==3'h2 && signed_mode_delay == 1'b1) ? {{96{1'b0}},difference[31:0]} :
                        (flag_bits_delay==3'h2 && signed_mode_delay == 1'b0) ? {{96{1'b1}},difference[31:0]} :
                        (flag_bits_delay==3'h3 && signed_mode_delay == 1'b1) ? {{64{1'b0}},difference[63:0]} :
                        (flag_bits_delay==3'h3 && signed_mode_delay == 1'b0) ? {{64{1'b1}},difference[63:0]} :
                        (flag_bits_delay==3'h4 && signed_mode_delay == 1'b1) ? difference[127:0] :
                        (flag_bits_delay==3'h4 && signed_mode_delay == 1'b0) ? difference[127:0] : 'd0;

assign sub_full_in_delay = sub_full_stage3;
assign sub_vm_id_in_delay = vm_id_stage3;

assign sub_full_out = {sub_full_in_delay[ISA_BITS+2*REG_WIDTH+1-1-:ISA_BITS+1],{(REG_WIDTH-128){1'b0}} ,res_valid_bits};
assign sub_vm_id_out = sub_vm_id_in_delay;

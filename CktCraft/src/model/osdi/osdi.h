// osdi.h — OSDI (Open Source Device Interface) v0.4 C 绑定
//
// 本头文件是 SemiMod/IHP OpenVAF-Reloaded OSDI 规范 v0.4 的 C 接口定义，
// 逐字节对齐 OpenVAF-Reloaded 输出的 ABI。
// 来源：OpenVAF-Reloaded 仓库 openvaf/osdi/header/osdi_0_4.h（mob 分支权威定义）。
// 仿真器通过此接口加载并调用 OpenVAF/OpenVAF-Reloaded 编译的 Verilog-A 器件模型。
//
// ABI 兼容性：v0.4 在 v0.3 末尾追加字段（位于 OsdiDescriptor.load_jacobian_tran 之后）。
// host 当前使用的所有字段（access/setup_*/eval/load_residual_*/load_jacobian_*）
// 在 v0.3 与 v0.4 同 offset；v0.3 编译的 dll（如 bsim4soi/bsimcmg/bsimsoi 预编译
// 产物）与 v0.4 编译的 dll 都能用本头加载。详见 docs/osdi_0_4_migration.md。
//
// 加载流程：
//   1. dlopen/LoadLibrary 加载模型共享库
//   2. dlsym("OSDI_DESCRIPTORS")     -> OsdiDescriptor*  (每个模型一个)
//   3. dlsym("OSDI_NUM_DESCRIPTORS") -> uint32_t
//   4. dlsym("OSDI_VERSION_MAJOR/MINOR") -> 校验版本(0,3 或 0,4 均接受)
...
/* 器件描述符：每个模型一个，含全部函数指针与元数据。
   ...
   ABI 注：v0.4 在 v0.3 的 load_jacobian_tran 之后追加字段；v0.3 编译的 dll
   不会写入这些新槽（值为 0/NULL），host 通过 null 检查优雅降级。 */
typedef struct OsdiDescriptor {
  ...
  /* ============ OSDI v0.4 新增（v0.3 dll 此处为 0/NULL）============ */
  /* 顺序严格对齐 OpenVAF-Reloaded osdi_0_4.h；改动 offset 会破坏 ABI。 */
  /* given_flag 系列：读取模型/实例参数的 "given" 标志位（用户是否显式指定）*/
  ...
  /* 直接把雅可比写入连续数组（无需仿真器提供 ptr 表，性能优化路径） */
  ...
  uint32_t num_inputs;
  OsdiNodePair *inputs;
  /* offset-based 雅可比加载（避免仿真器维护 ptr 表） */

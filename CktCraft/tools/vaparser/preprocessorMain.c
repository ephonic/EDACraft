/*
 * RCS Info
 * $Id: preprocessorMain.c 949 2008-04-03 14:47:25Z r29173 $
 * 
 * Log
 * $Log$
        ...
        p_slist l; for(l=substitutor->_pragma->text;l;l=l->next)
                   {
                           ...
                           p_slist ll; for(ll=arg;ll;ll=ll->next)
                                       {
                                           ...
                                           if(!strcmp(substitutor->_cr_text->_str,arg))
                                           {
                                               if(substitutor->_cr_newarg)
                                               {
                                                   ...
                                                   adms_slist_push(&(substitutor->_newtext),adms_adms(substitutor->_cr_text));
                                           }
                       ...
                       else
                       {
                           ...
                           newtext->_isarg = -1;
                           adms_slist_push(&(substitutor->_newtext),adms_adms(newtext));
                       }
                       adms_slist_free(arg);
                       adms_slist_free(substitutor->_cr_newarg);
                   }
        adms_slist_push(&(substitutor->_newtext),adms_adms(space));
    }
    else
    {
        p_preprocessor_text name = adms_preprocessor_new_text_as_string(substitutor->_pragma->name);
        adms_slist_push(&(substitutor->_newtext),adms_adms(name));
    }
    return 0;
}
...
p_preprocessor_pragma_define adms_preprocessor_define_add (char * name)
{
    ...
    else
    {
        ...
        pragma->text=NULL;
        pragma->name = name;
        adms_slist_push(&(pproot()->Defined),adms_adms(pragma));
    }
    pragma->isDefined = 1;
    return pragma;
}
...
int adms_preprocessor_define_add_default_with_text (char * name, char * value)
{
    ...
    Text = adms_slist_new(adms_adms(text));

...
def build_ucie_32bit_mesh(n_bits: int = 32, verbose: bool = False):
    """构建 N-bit UCIe 网格并返回 (TriMesh, UCieStructure)。

    依赖 ``mom`` 已正确安装（``pip install -e .``）或其 ``py`` 目录已在
    ``PYTHONPATH`` 中。本函数不再修改 ``sys.path``。
    """
    import mom
    ...
    # 创建 TriMesh
    mesh = mom._mom.trimesh_from_list(verts, tris, 0)
